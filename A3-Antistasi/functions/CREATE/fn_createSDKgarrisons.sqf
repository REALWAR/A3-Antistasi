...
then
{
	...
	then
	{
		...
		null = [_veh, "SDKFlag"] remoteExec ["A3A_fnc_flagaction", 0, _veh];

		if (_markerX in seaports)
		then { null = [_veh, "seaport"] remoteExec ["A3A_fnc_flagaction", [teamPlayer, civilian], _veh]; };
	};

	if ((_markerX in resourcesX) || {
		(_markerX in factories) })
	then
	{
		if !(_markerX in destroyedSites)
		then
		{
			if ((daytime > 8) && {
				(daytime < 18) })
			then
			{
				...
				{
					if (spawner getVariable _markerX != 2)
					then
					{
							...
							"Killed",
							{
								...
								_this spawn
								{
									...
									then
									{
										...
										null = ["TaskFailed", ["", format ["%1 Destroyed", _nameX]]] remoteExec ["BIS_fnc_showNotification", [teamPlayer, civilian]];
									};
								};
							}
						];
				...
				null = [leader _groupCiv, _markerX, "SAFE", "SPAWNED", "NOFOLLOW", "NOSHARE", "DORELAX", "NOVEH2"] execVM "scripts\UPSMON.sqf";//TODO need delete UPSMON link
			};
		};
	};
};
...
if (staticCrewTeamPlayer in _garrison)
then
{
...

	{
		...
		null = [_veh] execVM "scripts\UPSMON\MON_artillery_add.sqf";//TODO need delete UPSMON link
...
// Move riflemen into saved static weapons in area
{
	...
	then
	{
		...
		null = [_x] execVM "scripts\UPSMON\MON_artillery_add.sqf";//TODO need delete UPSMON link
	}
	else
	{
	...
	null = [_unit, _markerX] call A3A_fnc_FIAinitBases;
...
for "_i" from 0 to (count _groups) - 1 do
{
	...
	if (_i == 0)
	then { null = [leader _groupX, _markerX, "SAFE", "SPAWNED", "RANDOMUP", "NOVEH2", "NOFOLLOW"] execVM "scripts\UPSMON.sqf"; }
