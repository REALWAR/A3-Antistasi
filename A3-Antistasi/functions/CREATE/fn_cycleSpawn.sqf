...

{
	...
	then
	{
		...
		if (_vehicleType isKindOf "Car")
		then { _spawnParameter = [_marker, "Vehicle"] call A3A_fnc_findSpawnPosition; };
		if (_vehicleType isKindOf "Helicopter")
		then { _spawnParameter = [_marker, "Heli"] call A3A_fnc_findSpawnPosition; };
		if (_vehicleType isKindOf "Plane")
		then { _spawnParameter = [_marker, "Plane"] call A3A_fnc_findSpawnPosition; };

		//_spawnParameter = [getMarkerPos _marker, 0];
		if (_spawnParameter isEqualType [])
		then
		{
			_vehicle = createVehicle [_vehicleType, _spawnParameter #0, [], 0 , "CAN_COLLIDE"];
			...
			_vehicle setDir (_spawnParameter #1);
			...
			[
				"Killed",
				{
					...
					null = _this spawn
					{
						...
						[_marker, typeOf _unit, _id] call A3A_fnc_addRequested;
					};
				}
			];
	...
	then
	{
		...
		{
			_unitX = [_groupX, _x, (_spawnParameter #0), [], 5, "NONE"] call A3A_fnc_createUnit;
				...
				[
					"Killed",
					{
						...
						_this spawn
						{
							...
							_id = _unit getVariable "UnitIndex";
		...
		{
			_unitX = [_groupSoldier, _x, (_spawnParameter #0), [], 5, "NONE"] call A3A_fnc_createUnit;
			_allSoldiers pushBack _unitX;
...
if (count _allSoldiers != 0)
then { _sizePerUnit = _patrolSize / (count _allSoldiers); };
...
if (_sizePerUnit > 12500)
then
{
	diag_log "The area is to large, make it smaller";
	_patrolMarkerSize set [0, (_patrolMarkerSize #0) * (12500/_sizePerUnit)];
	_patrolMarkerSize set [1, (_patrolMarkerSize #1) * (12500/_sizePerUnit)];
};

_mainMarkerSize = getMarkerSize _marker;

if (((_patrolMarkerSize #0) < (_mainMarkerSize #0)) || {
	((_patrolMarkerSize #1) < (_mainMarkerSize #1)) })
then
{
