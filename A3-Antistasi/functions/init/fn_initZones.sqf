...
if ((toLower worldName) in ["altis", "chernarus_summer"])
then
{
...
if (debug)
then { diag_log format ["%1: [Antistasi] | DEBUG | initZones | Setting Spawn Points for %2.", servertime, worldname]; };
...
if (debug)
then { diag_log format ["%1: [Antistasi] | DEBUG | initZones | Building roads for %2.", servertime, worldname]; };
...
configClasses (configfile >> "CfgWorlds" >> worldName >> "Names") apply
{
	...
	then
	{
		...
		_roadsProv apply
		{
			...
			if (count _roadcon == 2)
			then { _roads pushBack (getPosATL _x); };
		};
	...
	else { _numCiv = (count (nearestObjects [_pos, ["house"], _size])); };
...
if (debug)
then { diag_log format ["%1: [Antistasi] | DEBUG | initZones | Roads built in %2.", servertime, worldname]; };
...
if (debug)
then { diag_log format ["%1: [Antistasi] | DEBUG | initZones | Setting up Radio Towers.", servertime]; };
...
private _replaceBadAntenna = {
	...
	if ((typeof _antenna) in ["Land_Communication_F", "Land_Vysilac_FM", "Land_A_TVTower_Base"])
	then
	{
		hideObjectGlobal _antenna;

		if (typeof _antenna == "Land_A_TVTower_Base")
		then
		{
...
then
{
	for "_i" from 0 to (count _posAntennas - 1) do
	{
		...
		if (count _antennaProv > 0)
		then
		{
			_antenna = _antennaProv #0;

			if (_i in _blacklistPos)
			then { _antenna setdamage 1; }
			else
			{
...
if (debug)
then { diag_log format ["%1: [Antistasi] | DEBUG | initZones | Broken Radio Towers identified.", servertime]; };
...
then
{
	for "_i" from 0 to (count _posBank - 1) do
	{
		...
		if (count _bankProv > 0)
		then
		{
			private _banco = _bankProv #0;
			banks = banks + [_banco];
		};
	};
};
...
if (isMultiplayer)
then { [petros, "hint", "Zones Init Completed"] remoteExec ["A3A_fnc_commsMP", -2] };
