...
if (_victim getVariable ["spawner", false])
then { _victim setVariable ["spawner", nil, true]; };
...
if ((hasACE) && {
	(isNull _killer) || {
	(_killer == _victim) }})
then { _killer = _victim getVariable ["ace_medical_lastDamageSource", _killer]; };
...
then
{
	...
	then
	{
		...
		if ((captive _killer) && {
			(_killer distance _victim < distanceSPWN) })
		then
		{
	...
	if (count weapons _victim < 1 && {
		!(_victim getVariable ["isAnimal", false]) })
	then
	{
	...
	else
	{
		...
		if (_victimSide == Occupants)
		then { [[0.5, 45], [0, 0]] remoteExec ["A3A_fnc_prestige", 2]; }
		else { [[0, 0], [0.5, 45]] remoteExec ["A3A_fnc_prestige", 2]; };
	};
}
else
{
	if (_victimSide == Occupants)
	then { [-0.25, 0, getPos _victim] remoteExec ["A3A_fnc_citySupportChange", 2]; }
	else { [0.25, 0, getPos _victim] remoteExec ["A3A_fnc_citySupportChange", 2]; };
};

private _victimLocation = _victim getVariable "markerX";
private _victimWasGarrison = true;

if (isNil "_victimLocation")
then
{
...
if ((_victimLocation != "") && {
	(sidesX getVariable [_victimLocation, sideUnknown] == _victimSide) })
then
{
	[typeOf _victim, _victimSide, _victimLocation, -1] remoteExec ["A3A_fnc_garrisonUpdate", 2];

	if (_victimWasGarrison)
	then { [_victimLocation, _victimSide] remoteExec ["A3A_fnc_zoneCheck", 2]; };
};
