...
null = [_unit, true] remoteExec ["setCaptive", 0, _unit];
...
{
	if !(_loadout select _i isEqualTo [])
	then { _boxX addWeaponWithAttachmentsCargoGlobal [_loadout select _i, 1]; };
};
...
if (side group _unit == Occupants)
then { null = [-2, 0, getPos _unit] remoteExec ["A3A_fnc_citySupportChange", 2]; }
else { null = [1, 0, getPos _unit] remoteExec ["A3A_fnc_citySupportChange", 2]; };
...
null = [3, format["Cleanup called for unit:%1", _unit], _filename] call A3A_fnc_log;
null = [_unit] spawn A3A_fnc_postmortem;

null = [3, format["Cleanup called for boxx:%1", _boxX], _filename] call A3A_fnc_log;
null = [_boxX] spawn A3A_fnc_postmortem;

sleep 3;
_unit allowDamage true;
//if (isMultiplayer) then {[_unit, false] remoteExec ["enableSimulationGlobal", 2]} else {_unit enableSimulation false};
null = [_unit, "captureX"] remoteExec ["A3A_fnc_flagaction", [teamPlayer, civilian], _unit];
