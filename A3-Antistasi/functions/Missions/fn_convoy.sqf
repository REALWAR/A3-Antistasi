...
private _dateFinal = [date #0, date #1, date #2, date #3, (date #4) + _timeXfin];
private _enddateNum = dateToNumber _dateFinal;

private _convoyTypes = [];

if ((_mrkDest in airportsX) or (_mrkDest in outposts))
then
{
	_convoyTypes = ["Ammunition", "Armor"];

	if ((_mrkDest in outposts) && {
		(((count (garrison getVariable [_mrkDest, []]))/2) >= [_mrkDest] call A3A_fnc_garrisonSize) })
	then { _convoyTypes pushBack "Reinforcements"; };
}
else
{
	if (_mrkDest in citiesX)
	then
	{
		if (sidesX getVariable [_mrkDest, sideUnknown] == Occupants)
		then { _convoyTypes = ["Supplies"]; }
		else { _convoyTypes = ["Supplies"]; };
	}
	else
	{
		if ((_mrkDest in resourcesX) || {
			(_mrkDest in factories) })
		then { _convoyTypes = ["Money"]; }
		else { _convoyTypes = ["Prisoners"]; };

		if (((count (garrison getVariable [_mrkDest, []]))/2) >= [_mrkDest] call A3A_fnc_garrisonSize)
		then { _convoyTypes pushBack "Reinforcements"; };
	};
};

if (_convoyType == "")
then { _convoyType = selectRandom _convoyTypes };
...
private _dateLimit = [date #0, date #1, date #2, date #3, (date #4) + _timeLimit];
private _dateLimitNum = dateToNumber _dateLimit;
_dateLimit = numberToDate [date #0, _dateLimitNum];//converts datenumber back to date array so that time formats correctly when put through the function
...
null = [_mrkOrigin, 30] call A3A_fnc_addTimeForIdle;
...
if (_route isEqualTo [])
then { _route = [_posOrig, _posDest]; }
else { _route deleteAt 0; };

// origin will be doubled

private _vecdir = (_route #0) vectorFromTo (_route #1);
private _dir = (_route #0) getDir (_route #1);
...
private _fnc_spawnConvoyVehicle = {
...

	{
		...
		if (side _group != civilian)
		then { null = [_x] call A3A_fnc_NATOinit; };
	...
	null = [_veh, _markName] spawn A3A_fnc_inmuneConvoy;			// NOTE: should not be called unless moving within 60s
...
private _fnc_spawnEscortVehicle = {
	...
	{
		if !(_typeVehEsc == vehFIAArmedCar)
		then
		{
			private _typeGroup = selectRandom groupsFIASquad;

			if (_typeVehEsc == vehFIACar)
			then { _typeGroup = selectRandom groupsFIAMid; };
...
{
	if (!_isMilitia)
	then { selectRandom vehNATOLightArmed }
	else { vehPoliceCar }
}
else { selectRandom vehCSATLightArmed };
...
{
	sleep 2;
	null = [] call _fnc_spawnEscortVehicle;
};
...
if (_convoyType == "Armor")
then {_vehObj lock 3};// else {_vehObj forceFollowRoad true};

if (_convoyType == "Prisoners")
then
{
	...
	for "_i" from 1 to (1+ round (random 11)) do
	{
		...
		null = [_unit] call A3A_fnc_reDress;
	};
};

if (_convoyType == "Reinforcements")
then
{
...
if ((_convoyType == "Money") || {
	(_convoyType == "Supplies") })
then
{
	...
	[
		"HandleDamage",
		{
			...
			if ((_selection find "wheel" != -1) && {
				((_projectile == "") || {side _source != teamPlayer}) && {
...
private _fnc_applyResults =
{
	...
	null = [_adjustBoss, theBoss] call A3A_fnc_playerScoreAdd;

	if (_sideX == Occupants)
	then { [[_aggroMod, _aggroTime], [0, 0]] remoteExec ["A3A_fnc_prestige", 2]; }
	else { [[0, 0], [_aggroMod, _aggroTime]] remoteExec ["A3A_fnc_prestige", 2]; };
	...
	null = [3, _eventText, "aggroEvent"] call A3A_fnc_log;
};

if (_convoyType == "Ammunition")
then
{
	waitUntil
	{
		sleep 1;
		(dateToNumber date > _enddateNum) || {
		(_vehObj distance _posDest < _distanceFromTargetForArrival) || {
		!(alive _vehObj) || {
		(driver _vehObj getVariable ["spawner", false]) && {
		(side group (driver _vehObj) == teamPlayer) }}}
	};

	if ((_vehObj distance _posDest < _distanceFromTargetForArrival) || {
		(dateToNumber date >_enddateNum) })
	then
	{
		null = [false, true, -1200*_bonus, -10*_bonus, -5, 60, "ammo"] call _fnc_applyResults;
	...
	{
		null = [true, false, 1800*_bonus, 5*_bonus, 25, 120, "ammo"] call _fnc_applyResults;
		[0, 300*_bonus] remoteExec ["A3A_fnc_resourcesFIA", 2];

		{
			if (isPlayer _x) then { null = [10*_bonus, _x] call A3A_fnc_playerScoreAdd; };
...
then
{
	...
	then
	{
		null = [false, true, -1200*_bonus, -10*_bonus, -5, 60, "armor"] call _fnc_applyResults;
		server setVariable [_mrkDest, dateToNumber date, true];
	}
	else
	{
		null = [true, false, 1800*_bonus, 5*_bonus, 20, 90, "armor"] call _fnc_applyResults;
		...
		null = [getPosASL _vehObj, _sideX, "", false] spawn A3A_fnc_patrolCA;
	};
};
...
then
{
	...
	if ((_vehObj distance _posDest < _distanceFromTargetForArrival) || {
		({alive _x} count _POWs == 0) || {
		(dateToNumber date > _enddateNum) }})
	then { null = [false, true, 0, -10*_bonus, -10, 60, "prisoner"] call _fnc_applyResults; };

	if (!(alive driver _vehObj) || {
		(driver _vehObj getVariable ["spawner", false]) && {
		(side group (driver _vehObj) == teamPlayer) }})
	then
	{
		...
		waitUntil
		{
			...
			({alive _x} count _POWs == 0) || {
			({(alive _x) && { (_x distance _posHQ < 50) }} count _POWs > 0) || {
			(dateToNumber date > _enddateNum)}
		};

		if (({alive _x} count _POWs == 0) || {
			(dateToNumber date > _enddateNum) })
		then { null = [false, false, 0, -10*_bonus, 20, 120, "prisoner"] call _fnc_applyResults; }
		else
		{
			_countX = {(alive _x) && { (_x distance _posHQ < 150) }} count _POWs;
			null = [true, false, 0, _bonus*_countX/2, 10, 120, "prisoner"] call _fnc_applyResults;
			...
			{
				null = [_countX, _x] call A3A_fnc_playerScoreAdd;
...
then
{
	...
	if ({(!alive _x) || { (captive _x) }} count _reinforcementsX == count _reinforcementsX)
	then
	{
		null = [true, false, 0, 5*_bonus, 10, 90, "reinforcement"] call _fnc_applyResults;
		[0, 10*_bonus, _posOrigin] remoteExec ["A3A_fnc_citySupportChange", 2];

		{
			if (_x distance _vehObj < 500)
			then { null = [10*_bonus, _x] call A3A_fnc_playerScoreAdd; };
		} forEach (allPlayers - (entities "HeadlessClient_F"));
	}
	else
	{
		null = [false, true, 0, -10*_bonus, -10, 60, "reinforcement"] call _fnc_applyResults;
		_countX = {alive _x} count _reinforcementsX;

		if (_countX <= 8) then {_taskState1 = "FAILED"};

		if (sidesX getVariable [_mrkDest, sideUnknown] != teamPlayer)
		then
		{
			_typesX = [];
...
then
{
	...
	if ((dateToNumber date > _enddateNum) || {
		(_vehObj distance _posDest < _distanceFromTargetForArrival) || {
		!(alive _vehObj) }})
	then
	{
		if ((dateToNumber date > _enddateNum) || {
			(_vehObj distance _posDest < _distanceFromTargetForArrival) })
		then { null = [false, true, -1200, -10*_bonus, -5, 60, "money"] call _fnc_applyResults; }
		else { null = [false, false, 1200, 0, -5, 60, "money"] call _fnc_applyResults; };
	};

	if ((driver _vehObj getVariable ["spawner", false]) && {
		(side group (driver _vehObj) == teamPlayer) })
	then
	{
		null = [getPosASL _vehObj, _sideX, "", false] spawn A3A_fnc_patrolCA;
		...
		if (!(alive _vehObj) || {
			(dateToNumber date > _enddateNum) })
		then { null = [false, false, 1200, 0, -5, 60, "money"] call _fnc_applyResults; };

		if (_vehObj distance _posHQ < 50)
		then
		{
			null = [true, false, 1200, 5*_bonus, 25, 120, "money"] call _fnc_applyResults;

			null = [0, 5000*_bonus] remoteExec ["A3A_fnc_resourcesFIA", 2];
...
then
{
	...
	then
	{
		null = [false, false, 0, -10*_bonus, 20, 120, "supply"] call _fnc_applyResults;
		null = [getPosASL _vehObj, _sideX, "", false] spawn A3A_fnc_patrolCA;
	};

	if ((dateToNumber date > _enddateNum) || {
		(_vehObj distance _posDest < 300) || {
		(driver _vehObj getVariable ["spawner", false]) and {
		(side group (driver _vehObj) == teamPlayer) }}})
	then
	{
		if ((driver _vehObj getVariable ["spawner", false]) && {
		...
		then
		{
			...
			waitUntil
			{
				...
				(_vehObj distance _posDest < 100) || {
				!(alive _vehObj) || {
				(dateToNumber date > _enddateNum) }}
			};

			if (_vehObj distance _posDest < 100)
			then
			{
				null = [true, false, 0, 5*_bonus, 10, 90, "supply"] call _fnc_applyResults;
			...
			{
				null = [false, false, 0, -10 * _bonus, -10, 60, "supply"] call _fnc_applyResults;
				[5 * _bonus, -10 * _bonus, _mrkDest] remoteExec ["A3A_fnc_citySupportChange", 2];
			};
		}
		else
		{
			null = [false, true, 0, -10 * _bonus, -10, 60, "supply"] call _fnc_applyResults;
...
null = ["CONVOY", [_textX, _taskTitle, _mrkDest], _posDest, _taskState] call A3A_fnc_taskUpdate;
null = ["CONVOY1", [format ["A convoy from %1 to %3, it's about to depart at %2. Protect it from any possible attack.", _nameOrigin, _displayTime, _nameDest], "Protect Convoy", _mrkDest], _posDest, _taskState1] call A3A_fnc_taskUpdate;
...
null = [600, "CONVOY"] spawn A3A_fnc_deleteTask;
null = [0, "CONVOY1"] spawn A3A_fnc_deleteTask;
...
{ null = [_x] spawn A3A_fnc_groupDespawner; } forEach _groups;
{ null = [_x] spawn A3A_fnc_VEHdespawner; } forEach _vehiclesX;
