...
_timeLimit = 120;
_dateLimit = [date #0, date #1, date #2, date #3, (date #4) + _timeLimit];
...
if (count _posCrash == 0)
then
{
	if (!isMultiplayer)
	then { { _x hideObject true } foreach (nearestTerrainObjects [_posCrashOrig, ["tree", "bush"], 50]) }
	else { {[_x, true] remoteExec ["hideObjectGlobal", 2]} foreach (nearestTerrainObjects [_posCrashOrig, ["tree", "bush"], 50]) };
...
sleep 15;
_typeVehX = if (_sideX == Occupants) then { vehNATOCargoTrucks #1} else {vehCSATTrucks #0 };
...
_vehCrewT = _vehicle #1;
{[_x] call A3A_fnc_NATOinit} forEach _vehCrewT;
_groupVehT = _vehicleT #2;
...
if ((not alive _heli) || {((taskState "DES") == "SUCCEEDED") || {(count (_vehicle getVariable ["SA_Tow_Ropes", []]) > 0)}})
then
{
	...
	if (typeOf _heli in vehCSATAir)
	then { [[0, 0], [15, 90]] remoteExec ["A3A_fnc_prestige", 2] };
	if (typeOf _heli in vehNATOAir)
	then { [[15, 90], [0, 0]] remoteExec ["A3A_fnc_prestige", 2] };
...
[0, "DES1"] spawn A3A_fnc_deleteTask;
