null = _this spawn
{
	...
	then
	{
		...
		if ((time > _groupX getVariable ["movedToCover", 0]) && {
			(behaviour leader _groupX != "COMBAT") && {
			(behaviour leader _groupX != "STEALTH") }})
		then
		{
			_groupX setVariable ["movedToCover", time + 120];
			{ null = [_x, _injurer] spawn A3A_fnc_unitGetToCover; } forEach units _groupX;
		};

		if ((_part == "") && {
			(_damage < 1) && {
			(_damage > 0.6) }})
		then { null = [_unit, _injurer] spawn A3A_fnc_unitGetToCover; };

		// Contact report generation for PvP players
		if ((_part == "") && {
			(side group _unit == Occupants) })
		then
		{
			...
			if ((_marker != "") && {
				(sidesX getVariable [_marker, sideUnknown] == Occupants) })
			then
			{
				...
				then
				{
					...
					null = [_marker, teamPlayer, side group _unit] remoteExec ["A3A_fnc_underAttack", 2];
				};
			};
		};
	};
...
if !(side _injurer == teamPlayer) exitWith {};
...
private _makeUnconscious =
{
	...
	if (isPlayer _unit)
	then { _unit allowDamage false; };

	null = [_unit, _injurer] spawn A3A_fnc_unconsciousAAF;
};
...
then
{
	...
	then
	{
		...
		else
		{
			...
			if (_overall > 0.5)
			then { _unit removeAllEventHandlers "HandleDamage"; }
			else
			{
	...
	else
	{
		...
		if ((_damage > 0.25) && {
			(_unit getVariable ["helping", false]) })
...
else
{
	...
	then
	{
		...
		then { null = [_unit, _injurer] spawn _makeUnconscious; };
	};
};
