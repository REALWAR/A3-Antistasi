null = _this spawn
{
	...
	then
	{
		...
		if ((isPlayer _instigator) && {
			(_unit != _instigator) && {
			(side group _instigator == teamPlayer) && {
		...
		then
		{
			null = [
				format
				[
					"%1 was injured by %2 (UID: %3), %4m from HQ",
					name _unit,
					name _instigator,
					getPlayerUID _instigator,
					_unit distance2D posHQ
				]
		...
		then
		{
			...
			then
			{
				...
				then
				{
					...
					null = [_marker, side group _injurer, side group _unit] remoteExec ["A3A_fnc_underAttack", 2];
				};
			};
		};
	};
	...
	else
	{
		...
		then
		{
			...
			then
			{
				...
				if (isNull _helped)
				then { null = [_unit] spawn A3A_fnc_askHelp; };
			};
		};
	};
}
