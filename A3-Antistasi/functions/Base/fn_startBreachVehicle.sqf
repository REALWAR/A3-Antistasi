...
if (!isPlayer _caller)
exitWith { null = ["Breach Vehicle", "Only players are currently able to breach vehicles!"] call A3A_fnc_customHint; };

//Only engineers should be able to breach a vehicle
private _isEngineer = _caller getUnitTrait "engineer";

if (!_isEngineer)
exitWith { null = ["Breach Vehicle", "You have to be an engineer to breach a vehicle!"] call A3A_fnc_customHint; };

if (!alive _vehicle)
exitWith
{
	null = ["Breach Vehicle", "Why would you want to breach a destroyed vehicle?"] call A3A_fnc_customHint;
...
if (count _aliveCrew == 0)
exitWith
{
	null = ["Breach Vehicle", "There is no living crew left, no need for breaching!"] call A3A_fnc_customHint;
	_vehicle lock false;
	_vehicle removeAction _actionID;
};

if (side (_aliveCrew #0) == teamPlayer)
exitWith
{
	null = ["Breach Vehicle", "You cannot breach a vehicle which is controlled by the rebels!"] call A3A_fnc_customHint;
	_vehicle removeAction _actionID;
};
...
{
	private _mag =_x;
	_index = _magazineArray findIf {(_x #0) == _mag};

	if (_index == -1)
	then { _magazineArray pushBack [_mag, 1]; }
	else
	{
		private _element = _magazineArray select _index;
		_element set [1, (_element #1) + 1];
	};
} forEach _magazines;

//Abort if no explosives found
if (_magazineArray isEqualTo [])
exitWith { null = ["Breach Vehicle", "You carry no explosives. You will need some to breach vehicles!"] call A3A_fnc_customHint; };
...
{
	_explosive = _explo #0;
	_explosiveCount = _explo #1;
};

if (_explosiveCount == 0)
exitWith { null = ["Breach Vehicle", "You don't have the right explosives, check the briefing notes to see what you need!"] call A3A_fnc_customHint; };
...
private _action = _caller addAction ["Cancel Breaching", {(_this #1) setVariable ["cancelBreach", true]}, nil, 6, true, true, "", "(isPlayer _this) && (_this == vehicle _this)"];
_vehicle removeAction _actionID;

_caller addEventHandler [
	"AnimDone",
	{
		private _caller = _this #0;
		...
		then { _caller playMoveNow selectRandom medicAnims; }
		else
		{
...
//TODO there was something about that on the optimisation page, look it up
if (!(alive _vehicle) || {
	(_caller != vehicle _caller) || {
	(_caller distance _vehicle >= 8) || {
	!([_caller] call A3A_fnc_canFight) || {
...
exitWith
{
	null = ["Breach Vehicle", "Breaching cancelled"] call A3A_fnc_customHint;
	_caller setVariable ["cancelBreach", nil];

	if (alive _vehicle)
	then { null = _vehicle call A3A_fnc_addActionBreachVehicle; };
};

//Remove the correct amount of explosives
for "_count" from 1 to _explosiveCount
do { _caller removeMagazineGlobal _explosive; };
...
null = ["Breach Vehicle", "Breaching in 10 seconds."] call A3A_fnc_customHint;
sleep 10;
...
if (((damage _vehicle) + _damageDealt) > 0.9)
exitWith
{
...

{
	...
	then
	{
		...
		null = [_x] remoteExec ["A3A_fnc_surrenderAction", _x];		// execute local to crewman
	}
	else
	{
...
null = [_vehicle, teamPlayer, true] call A3A_fnc_vehKilledOrCaptured;
